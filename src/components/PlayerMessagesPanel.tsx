...
const PlayerMessagesPanel = ({
  ...
  const fetchAdmins = async () => {
    try {
      setLoading(true);
      console.log("Fetching admin contacts from database...");
      ...
      if (error) {
        console.error("Failed to fetch admins from database:", error);
        throw error;
      }

      // Transform the data to match AdminContact interface
      const adminContacts: AdminContact[] = (data || []).map((admin) => ({
        name: admin.name,
        email: admin.email,
      }));

      setAdmins(adminContacts);
      console.log("Fetched admins from database:", adminContacts);
    } catch (error) {
