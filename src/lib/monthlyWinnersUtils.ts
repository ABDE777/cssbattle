...
export const saveMonthlyWinners = async (): Promise<boolean> => {
  try {
    ...
    // Prepare winner data
    const winnersData: MonthlyWinnerInsert[] = players.map((player, index) => ({
      player_id: player.id,
...
export const getMonthlyWinners = async (month?: string): Promise<MonthlyWinner[] | null> => {
  try {
    let query = supabase
      .from("monthly_winners" as any)
      .select("*")
